...
export async function centerCropToSquare(uri) {
  const data = await Skia.Data.fromURI(uri);
  const img = Skia.Image.MakeImageFromEncoded(data);
  if (!img) return uri;
  ...
  const outSurf = Skia.Surface.Make(size, size);
  if (!outSurf) return uri;
  outSurf.getCanvas().drawImageRect(
    img,
    Skia.XYWHRect(cropX, cropY, size, size),
    Skia.XYWHRect(0, 0, size, size),
    Skia.Paint()
  );
  outSurf.flush();

  const bytes = outSurf.makeImageSnapshot().encodeToBytes(ImageFormat.JPEG, 92);
  if (!bytes) return uri;

  const b64 = encodeBase64(bytes.buffer);
  const outPath = `${FileSystem.cacheDirectory}square_${Date.now()}.jpg`;
  await FileSystem.writeAsStringAsync(outPath, b64, { encoding: FileSystem.EncodingType.Base64 });
  return outPath;
}
