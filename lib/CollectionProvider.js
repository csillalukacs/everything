...
import { acquiredFields, imagePathsForItem, ITEM_IMAGES_BUCKET, thumbOf } from '../shared/items';
...
export function CollectionProvider({ children }) {
  ...
  async function uploadLocalPhoto(photoUri) {
    ...
    const ts = Date.now();
    const path = `${session.user.id}/${ts}.${ext}`;
    const thumbPath = `${session.user.id}/${ts}_thumb.webp`;
    ...
    if (thumbBytes) {
      const { error: thumbErr } = await supabase.storage
        .from('item-images')
        .upload(thumbPath, thumbBytes, { contentType: 'image/webp', cacheControl: '31536000, immutable' });
  ...
  async function updateItem(id, name, photoOrUri, tagNames, isPrivate, description, acquired, ocrText, previousImages, imageAddedAt) {
    ...
    let thumb_url = items.find(i => i.id === id)?.thumb_url ?? null;
