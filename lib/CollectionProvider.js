...
import { acquiredFields, imagePathFromUrl, imagePathsForItem, thumbOf, isRetired, sameTagSet } from '../shared/items';
...
import { fetchBlockedIds, fetchBlockedByIds, blockUser, unblockUser, submitReport } from '../shared/moderation';
...
import { fetchUnreadNotificationCount, markNotificationsRead, subscribeToNotifications } from '../shared/notifications';
...
export function CollectionProvider({ children }) {
  ...
  // Deferred delete: items vanish from the grid immediately, but the real
  // DB + storage delete is held for a few seconds so it can be undone.
  const [pendingDelete, setPendingDelete] = useState(null); // { items } | null
  const pendingDeleteRef = useRef(null); // { items, timer }
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      fetchBlockedByIds(supabase)
        .then(ids => { if (!cancelled) setBlockedByIds(new Set(ids)); })
      ...
      fetchLikedItemIds(supabase, session.user.id)
        .then(ids => { if (!cancelled) setLikedItemIds(new Set(ids)); })
  ...
  useEffect(() => {
    if (!session) return;
    return subscribeToNotifications(supabase, session.user.id, refreshNotifications);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [session]);

  // Register this device for OS push once signed in. No-op unless this is a
  // distribution build with push enabled (see lib/push / featureFlags).
  useEffect(() => {
    if (!session) return;
    registerForPush(supabase, session.user.id);
  }, [session]);

  // Fallback for events missed while the realtime socket was down: refresh the
  // unread count whenever the app returns to the foreground.
  useEffect(() => {
    if (!session) return;
  ...
  // Favorite another user's item (optimistic). The DB rejects favoriting your own
  // item or a blocked user's, so this is a no-op on those.
  async function likeItem(itemId) {
    if (!session || !itemId || likedItemIds.has(itemId)) return;
    setLikedItemIds(prev => new Set(prev).add(itemId));
    try {
      await addLike(supabase, { userId: session.user.id, itemId });
    } catch (e) {
  ...
  async function unlikeItem(itemId) {
    ...
    try {
      await removeLike(supabase, { userId: session.user.id, itemId });
    } catch (e) {
      console.error('unlikeItem error:', e);
      setLikedItemIds(prev => new Set(prev).add(itemId));
    }
  }
  ...
  async function updateItem(id, name, photoOrUri, tagNames, isPrivate, description, acquired, ocrText, previousImages, imageAddedAt) {
    ...
    const resolved = await ensureTags(tagNames);
    if (!resolved) return null;
    if (!sameTagSet(current?.tags ?? [], resolved))
      await setItemTags(data.id, resolved.map(t => t.id));
  ...
  // Perform the real deletion for a set of items: junctions, rows, then storage.
  function commitDelete(targets) {
    if (!targets || targets.length === 0) return;
    const ids = targets.map(i => i.id);
    supabase.from('item_tags').delete().in('item_id', ids).then(({ error }) => {
      if (error) console.error('commitDelete item_tags error:', error);
    });
    supabase.from('items').delete().in('id', ids).then(({ error }) => {
      if (error) { console.error('commitDelete items error:', error); return; }
      deleteStorageForItems(targets);
    });
  }

  // Commit any held deletion right away (before a new one supersedes it).
  function flushPendingDelete() {
    ...
    pendingDeleteRef.current = null;
    setPendingDelete(null);
    commitDelete(pending.items);
  }

  // Remove items from the grid now; hold the real delete so it can be undone.
  function scheduleDelete(targets) {
    if (targets.length === 0) return;
    flushPendingDelete();
    const ids = new Set(targets.map(i => i.id));
    const activeCount = targets.filter(i => !isRetired(i)).length;
    setItems(prev => prev.filter(i => !ids.has(i.id)));
    setItemCount(c => (c == null ? null : Math.max(0, c - activeCount)));
    const timer = setTimeout(() => {
      const pending = pendingDeleteRef.current;
      pendingDeleteRef.current = null;
      setPendingDelete(null);
      if (pending) commitDelete(pending.items);
    }, UNDO_WINDOW_MS);
    pendingDeleteRef.current = { items: targets, timer };
  ...
  function undoDelete() {
    ...
    const activeCount = restored.filter(i => !isRetired(i)).length;
    setItems(prev => [...restored.filter(r => !prev.some(p => p.id === r.id)), ...prev]);
    setItemCount(c => (c == null ? null : c + activeCount));
  }
  ...
  async function batchEditItems(ids, { addTags = [], acquiredPatch = null, removeTagId = null } = {}) {
